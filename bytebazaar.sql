...
CREATE TABLE reviews(
	...
	reviewRating INT,
	reviewWriterID INT FOREIGN KEY REFERENCES users(userID) ,
...
INSERT INTO users(userEmail, userName, userPassword) VALUES ('admin@bytebazaar.com','Admin','admin123!')  --admin is neither buyer nor seller.
...
SELECT reviewText, reviewRating, userName, reviewProductID FROM reviews JOIN users ON(users.userID=reviews.reviewWriterID) WHERE reviewProductID=1;
...
--Getting admin
SELECT * FROM users WHERE userEmail='' AND userID NOT IN (SELECT buyers.buyerID FROM buyers) AND userID NOT IN (SELECT sellers.sellerID FROM sellers)